...
<style>
/* ===== 전체 컨테이너 ===== */
.mp_mypage_container { display: flex; width: 100%; min-height: 85vh; padding: 40px 70px; gap: 40px; }
.chat_main_content { flex: 1; border: 1px solid #e0e0e0; border-radius: 16px; padding: 20px; background: #fff; }
.chat_room_wrapper { display:flex; gap:20px; width:100%; min-height:70vh; }
.chat_friend_list { width: 30%; background: #fffaf3; border: 1px solid #eee; border-radius: 12px; display: flex; flex-direction: column; justify-content: flex-start; }
.chat_list_header { padding: 15px 20px; font-weight: 600; color: #c89f6d; border-bottom: 1px solid #eee; background: #fff; }
.chat_friend_items { flex: 1; overflow-y: auto; padding: 10px 0; }
.chat_friend_card { display: flex; align-items: center; gap: 10px; padding: 10px 15px; cursor: pointer; border-bottom: 1px solid #f0e6da; transition: 0.25s; border-radius: 12px; background: inherit; }
.chat_friend_card:hover { background: #faf4ed; }
.chat_friend_card.active { background: #c89f6d; color: #fff; }
.chat_friend_card img.friend_profile_img { width: 45px; height: 45px; border-radius: 50%; object-fit: cover; }
...
.chat_paging { display: flex; justify-content: center; gap: 6px; padding: 10px; border-top: 1px solid #eee; background: #fffaf3; }
.chat_paging button { background: #f5f1ec; border: none; border-radius: 6px; padding: 5px 10px; cursor: pointer; color: #444; font-weight: 600; transition: 0.2s; }
...
.chat_msg { max-width: 70%; padding: 10px 14px; border-radius: 18px; word-break: break-word; font-size: 14px; line-height: 1.4; position: relative; display: inline-block; }
.chat_msg.left { background: #eee; color: #333; align-self: flex-start; border-top-left-radius: 0; }
...
.chat_content .loading { text-align:center; padding:20px; color:#999; font-size:14px; }
.chat_content .error { text-align:center; padding:20px; color:#d9534f; font-size:14px; }
</style>
...
function selectFriend(element, friendId, friendNickname, roomId){
    ...
    .catch(err => {
        console.error(err);
        content.innerHTML = '<div class="error">메시지를 불러오지 못했습니다.</div>';
    });
}
...
function sendMessage(){
    ...
    const roomId = document.querySelector('.chat_friend_card.active')?.dataset.roomid;
    if(!roomId) { alert('대화할 친구를 선택해주세요.'); return; }

    const data = { roomId: roomId, senderId: loginUserId, content: input.value };

    fetch('<%=request.getContextPath()%>/Chat?t_gubun=sendMessageAjax', {
        method: 'POST',
